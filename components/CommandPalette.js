...
export default function CommandPalette({ navigation }) {
   ...
   // const navPage = locale==='vi' ? navigation.pages_vi : navigation.pages_en
   const navPage = locale==='vi' ? navigation.pages_vi : navigation.pages_en

   const filterednavigation = query
      ? navPage.filter((page) => page.name.toLowerCase().includes(query.toLocaleLowerCase()))
      : navPage
   return (
      <>
         <motion.button
            className="p-3 cursor-pointer transition-all duration-200 ease-in-out z-30"
            type="button"
         ...
         <Transition.Root show={isOpen} as={Fragment} afterLeave={() => setQuery('')}>
            <Dialog onClose={setIsOpen} className="fixed inset-0 z-30 overflow-y-auto p-12 pt-[20vh]">
               <Transition.Child
