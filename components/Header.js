...
export default function Header() {
  ...
  return (
    <>
      <header
        className={`fixed w-full bg-transparent ${isTop
          ? 'border-none'
      ...
      >
        <nav className="mx-auto flex w-full max-w-5xl items-center justify-between px-2 py-2 xl:px-0">
          <div className="flex w-full items-center justify-between text-base leading-5">
            <div className="hidden sm:block">
              <Link
                key={'tqt'}
                href={'/'}
                className="mr-4 text-gray-900 dark:text-gray-300 dark:hover:text-gray-100 xl:first:pl-0 sm:py-4"
              >
                <span className='title-page'>Tuantq</span>
              </Link>
              {headerNavLinks.map((link) => (
                <Link
                  key={link.title}
                  href={link.href}
                  className="link-underline mx-4 font-medium text-gray-900 dark:text-gray-300 dark:hover:text-gray-100 xl:first:pl-0 sm:py-4"
                >
