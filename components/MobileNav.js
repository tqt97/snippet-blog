...
const MobileNav = ({ navShow, onToggleNav }) => {
  ...
  return (
    <div className="sm:hidden">
      <div
        className={`fixed w-full h-screen top-12 right-0 bg-white dark:bg-violet-1000 z-40 ease-in-out duration-500 ${navShow ? 'translate-x-0' : 'translate-x-full'
      ...
      >
        ...
        <nav className="mt-8 relative space-y-8 z-40">
          <Link
            key={'tqt'}
            href={'/'}
            className="px-12 text-xl font-semibold leading-8 tracking-wide text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white"
          >
